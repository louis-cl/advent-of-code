...
Template adapted from https://github.com/Tomcat-42/aoc.zig-template and https://github.com/SpexGuy/Zig-AoC-Template/

Run `zig build day1` to run day1.zig
Run `zig build day1_test` to run tests in day1.zig
Run `zig build day1_install` to generate `./zig-out/bin/day1`

For speed, build for fast release `zig build day1_install -Doptimize=ReleaseFast`
Run executable with `./zig-out/bin/day1`
Benchmark with `hyperfine --shell=none --warmup 3 './zig-out/bin/day1`

A makefile simplifies:
- running: `make run_1`
